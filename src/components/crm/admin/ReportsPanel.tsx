...
import {
  ...
  requestExport, type AdminDashboard, type ApiProject, type ApiInvoice,
} from "@/lib/crmApi";
  ...
  return (
    <motion.div variants={fadeIn} initial="hidden" animate="visible" className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-xl font-bold text-neutral-900" style={{ fontFamily: "'Sora', sans-serif" }}>Reports & Analytics</h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: "Active Projects",    value: dash?.activeProjectCount ?? projects.filter((p) => !["completed","cancelled","archived"].includes(p.projectStatus??""  )).length, color: "#1E3A8A", bg: "#EFF6FF" },
          { label: "Overdue Projects",   value: dash?.overdueProjectCount ?? projects.filter((p) => p.isOverdue).length,  color: "#DC2626", bg: "#FEF2F2" },
          { label: "Pending Approvals",  value: dash?.pendingApprovalsCount ?? 0,                                          color: "#B45309", bg: "#FEF3C7" },
          { label: "New Leads Today",    value: dash?.newLeadsToday ?? 0,                                                  color: "#6D28D9", bg: "#F5F3FF" },
        ].map((m) => (
          <div key={m.label} className="rounded-2xl p-5 text-center"
            style={{ background: "white", border: "1px solid #E8EDF3" }}>
            <p className="text-3xl font-bold mb-1" style={{ fontFamily: "'Sora', sans-serif", color: m.color }}>{m.value}</p>
      ...
      <div className="rounded-2xl p-5" style={{ background: "white", border: "1px solid #E8EDF3" }}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
          {[
            { label: "This Month",  value: fmtCurrency(dash?.revenueThisMonth ?? 0),  color: "#1E3A8A" },
            { label: "Last Month",  value: fmtCurrency(dash?.revenueLastMonth ?? 0),  color: "#6D28D9" },
            { label: "Total Billed",value: fmtCurrency(totalBilled),                  color: "#B45309" },
            { label: "Collected",   value: fmtCurrency(totalPaid),                    color: "#15803D" },
          ].map((r) => (
            <div key={r.label} className="rounded-xl p-3" style={{ background: "#F8FAFC" }}>
              ...
              <p className="text-base font-bold" style={{ fontFamily: "'JetBrains Mono', monospace", color: r.color }}>{r.value}</p>
            </div>
          ))}
        </div>
        <div>
          <div className="flex items-center justify-between mb-1.5">
            ...
            <span className="text-xs font-bold" style={{ color: collectionPct >= 70 ? "#15803D" : "#B45309" }}>{collectionPct}%</span>
          </div>
          <div className="h-2 rounded-full overflow-hidden" style={{ background: "#F1F5F9" }}>
            <div className="h-full rounded-full" style={{ width: `${collectionPct}%`, background: "linear-gradient(90deg, #1E3A8A, #22C55E)" }} />
          </div>
        </div>
      </div>
            ...
            return (
              <div key={status}>
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs font-semibold capitalize text-neutral-700">{status.replace(/_/g, " ")}</span>
